...
def custom_dis(v1, v2):
    if len(v1)!=len(v2):
        print "error"
        return -1
    v3 = [int(v1[i]) & int(v2[i]) for i in range(len(v1))]
    if sum(v3):
        return 1.0/sum(v3)
    else:
        return sys.maxint
...
class KMEANS:
    ...
    def cluster(self, X, k, init = 'k-means++', coef_dict = {}):
        ...
        r = self.kcluster(X, distance = custom_dis, k = 3)
        print r
    ...
    def kcluster(self, rows,distance,k=4):# Cited from Programming Collective Intelligence 
...
if __name__=="__main__":
    ...
    data_file = os.path.join(props["output_path"], props["result"].replace('Y',props["featureid"])) #大表
    log = os.path.join(props["output_path"], props["cluster_log"].replace('Y', props['featureid']))
    cluster_result = os.path.join(props["output_path"], props["cluster_result"].replace('Y',props["featureid"]).replace('X',iter_n))
