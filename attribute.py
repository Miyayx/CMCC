...
def run(attr_file):
    ...
    props = read_properties(os.path.join(BASEDIR, FILE_FILE))

    links,linknum = get_link(os.path.join(BASEDIR, props['outlink']))
    inlinks,inlinknum = get_link(os.path.join(BASEDIR, props['inlink']))
...
if __name__=="__main__":
    config = read_file_config(os.path.join(BASEDIR,FILE_FILE))

    run(os.path.join(RESULT_PATH, config["others_output_path"])+config["attribute_output"]+"_"+"features1.dat")
