...
def exist_val(data, origin, fn = None, props={}):
    """
    判断样本都有哪些特征存在非0特征值
    Args:
    --------------------------------------
    ...
    index_list = []
    syns = get_synonym_words(os.path.join(BASEDIR, props['synonym_dict']))
...
def validation(featurefile, options, props):
    """
    featurefile: 特征文件名
    options格式：
    options = {
    ...
    with open(featurefile) as f:
        ...
        if options.has_key("section"):
            ...
            exist_val(f_d, sample_sl, options['section']['left_file'], props)
        ...
        if options.has_key("block"):
            ...
            exist_val(f_d, sample_bl, options['block']['left_file'], props )
...
def run(props, Y):
    ...
    validation(feature_file, options, props)
